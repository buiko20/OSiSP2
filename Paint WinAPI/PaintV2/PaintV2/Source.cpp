...
int GetOpenFileNameFromOpenFileDialog(PTCHAR filePath, HWND hWndDialogOwner);
int GetSaveFileNameFromSaveFileDialog(PTCHAR filePath, HWND hWndDialogOwner);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR cmdLine, int cmdShow)
{
	...
	windowsClassMain.lpszMenuName = MAKEINTRESOURCE(IDR_MAIN_MENU);
...
LRESULT CALLBACK WindowProcedure_Main(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
	{
		...
		int userAnser = MessageBox(hWnd, text, Òaption, styles);

		if (userAnser == IDYES)
		{
			SendMessage(hWnd, WM_COMMAND, MAKEWPARAM(IDM_FILE_SAVE, 0), (LPARAM)hWnd);
			return DefWindowProc(hWnd, message, wParam, lParam);
		}
		if (userAnser == IDNO)
		{
	...
	std::vector<DrawPictureData>* drawPictureDataArray, HDC hMemoryDCDraw, HDC hMemoryDCZoom)
{
	...
	case IDM_FILE_OPEN:
	{
		...
		::memset(gv_openFileNameBuffer, 0, sizeof(gv_openFileNameBuffer));
		if (GetOpenFileNameFromOpenFileDialog(gv_openFileNameBuffer, gv_hWndMainWindow))
		{
			...
			OpenEnhancedMetafile(hMemoryDCDraw, gv_openFileNameBuffer);
			ZoomPictureAndDisplay(hDC, hMemoryDCDraw, hMemoryDCZoom, 0, 0, 0, NULL, NULL);
	...
	case IDM_FILE_SAVE:
	{
		if ((gv_isFileOpen) || (isSavedAs))
		{
			SaveEnhancedMetafile(gv_openFileNameBuffer);
			gv_isFileSaved = 1;
			break;
		}
	}
	...
	{
		TCHAR fileNameBuffer_Save_As[MAX_PATH] = { 0 };

		if (GetSaveFileNameFromSaveFileDialog(fileNameBuffer_Save_As, gv_hWndMainWindow))
		{
			SaveEnhancedMetafile(fileNameBuffer_Save_As);
			::memset(gv_openFileNameBuffer, 0, sizeof(gv_openFileNameBuffer));
			lstrcpy(gv_openFileNameBuffer, fileNameBuffer_Save_As);
			gv_isFileSaved = 1;
			isSavedAs = 1;
		}
		else
	...
	case IDM_HELP_ABOUT:
	{
		TCHAR messageAbout[] = _TEXT("Logo: BSUIR\nProgram name: Paint\nAuthor: Artyom Buiko\nYear: 2017");
		TCHAR messageBoxCaption[] = _TEXT("About");
		MessageBox(gv_hWndMainWindow, messageAbout, messageBoxCaption, MB_OK | MB_ICONINFORMATION);
		break;
	}
	case IDM_SETTINGS:
	{
		if (DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG_SETTINGS), gv_hWndMainWindow, DialogProcedure_Settings))
		{
			// ok button pressed. 
		}
		else
	...
	case ID_TEXT:
	{
		...
		if (DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG_TEXT), gv_hWndMainWindow, DialogProcedure_Text))
		{
			// ok button pressed.
			(*figureToDraw) = ID_TEXT;
		}
		else
		{
			// cancel button pressed.
			(*figureToDraw) = ID_CURSOR;
		}
...
int GetOpenFileNameFromOpenFileDialog(PTCHAR filePath, HWND hWndDialogOwner)
{
	...
	openFileName.hwndOwner = hWndDialogOwner;
	openFileName.lpstrFile = fileNameBuffer;
	// Set lpstrFile[0] to '\0', so that GetOpenFileName does not 
	// use the contents of szFile to initialize itself.
	openFileName.lpstrFile[0] = '\0';
	openFileName.nMaxFile = sizeof(fileNameBuffer);
	openFileName.lpstrFilter = _TEXT("All *.*\0*.*\0Enhanced Metafile *.emf\0*.emf\0");
	...
	{
		lstrcpy(filePath, fileNameBuffer);
		return 1;
	}
